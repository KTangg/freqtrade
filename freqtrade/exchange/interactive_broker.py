...
"""
Interactive Broker Exchanges support
"""
import asyncio
import inspect
import logging
...
class InteractiveBroker:
    ...
    def __init__(self, config: Config, *, exchange_config: Optional[ExchangeConfig] = None,
        ...
        # Initialize ib objects
        ib_config = self._ib_config
        ib_config = deep_merge_dicts(exchange_conf, ib_config)

        self._api = self._init_ib(ib_config)
    ...
    def __del__(self):
        """
        Destructor - clean up and disconnect from IB
        """
        self.close()

    def close(self):
        logger.debug("IBConnection object destroyed, disconnecting from IB")
        if self._api.isConnected():
            logger.debug("Disconnecting from Interactive Brokers.")
    ...
    def _init_ib(self, config: Dict[str, Any]) -> IB:
        """
        Initialize IB with given config and return a valid
        IB instance.
        """
        ib = IB()
    ...
    @property
    def _ib_config(self) -> Dict:
        # Parameters to add directly to ib initialization.
        return {}

    @property
    def name(self) -> str:
        """exchange Name (from ib)"""
    ...
    def validate_pairs(self, pairs: List[str]) -> None:
        ...
        if not self._api.isConnected():
            raise OperationalException(
                    f"Pair couldn't be validated since IB is disconnected"
                    )

        # NOTE only support static pair since ib API couldn't list of all contract
        invalid_pairs = []
        for pair in pairs:
            # NOTE freqtrade has BaseCurrency/QuoteCurrency format for pairs
            if len(pair.split('/')) < 2:
                raise OperationalException(
                                f'Invalid pair format {pair} only accept SYMBOL/USD format'
                            )
            ...
            # Empty detail = No contract on exchange
            if len(l_detail) == 0:
                raise OperationalException(
    ...
    # NOTE IB only support USD quote with stock symbol base
    ...
    def validate_timeframes(self, timeframe: Optional[str]) -> None:
        ...
        """
        small_timeframe = ['1s', '5s', '10s', '15s', '30s']

        if not timeframe:
            raise OperationalException("Empty time frame")
        elif timeframe not in self.timeframes:
            raise OperationalException(
                f"Invalid timeframe '{timeframe}'. This exchange supports: {self.timeframes}")
        elif timeframe in small_timeframe:
    ...
    def exchange_has(self, endpoint: str) -> bool:
        ...
        Wrapper around IB 'has' attribute
    ...
    def get_precision(number: float) -> int:
        ...
        # If there is no decimal portion, return 0
        if len(parts) == 1:
            return 0
